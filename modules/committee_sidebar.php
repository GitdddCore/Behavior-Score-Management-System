...
/* 盾牌图标特殊颜色 */
.sidebar-title .fa-shield-halved { 
    font-size: 20px; 
...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-nav">
        ...
        <!-- 登出按钮 -->
        <div class="nav-item" data-page="logout" id="logoutBtn" onclick="handleLogout()">
...
function performLogout() {
    ...
    // 根据当前页面位置确定请求URL
    let requestUrl = 'modules/committee_sidebar.php';
    if (currentPath.includes('/modules/')) {
        requestUrl = 'committee_sidebar.php';
    } else if (currentPath.includes('/page/committee/')) {
        requestUrl = '../../modules/committee_sidebar.php';
    } else if (currentPath.includes('/committee/')) {
        requestUrl = '../../modules/committee_sidebar.php';
    }
    ...
    console.log('完整请求URL:', window.location.origin + '/' + requestUrl.replace(/^\.\.\//, ''));
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.status === 'success') {
                        console.log('登出成功，准备跳转到:', '/index.php');
                        // 使用绝对路径跳转到首页
                        window.location.href = '/index.php';
                    } else {
                        console.error('登出失败，服务器返回错误:', response.message);
                        throw new Error(response.message || '登出失败');
                    }
                } catch (e) {
                    console.error('JSON解析错误或其他错误:', e);
                    console.error('原始响应文本:', xhr.responseText);
                    
                    // 恢复按钮状态
                    setLogoutLoading(false);
                    const errorMsg = '登出失败，请重试 (解析错误: ' + e.message + ')';
                    if (typeof window.notification !== 'undefined') {
                ...
                // 恢复按钮状态
                setLogoutLoading(false);
                const errorMsg = '登出失败，请重试 (HTTP ' + xhr.status + ': ' + xhr.statusText + ')';
                if (typeof window.notification !== 'undefined') {
                    window.notification.error(errorMsg);
                } else {
                    alert(errorMsg);
